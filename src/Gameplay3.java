 ...
 * @author Siddh
 */
class GamePlay3 extends JPanel implements KeyListener, ActionListener  {
	...
	public GamePlay3() {
          try {
                background = ImageIO.read(getClass().getResource("ground3.jpg"));
...
class MapGenerator3 {
	...
	public void draw(Graphics2D g) {
		...
		for (int i = 0; i < map.length; i++) {
			for (int j=0; j< map[0].length;j++) {
				if(map[i][j] > 0) {
					g.setColor(new Color(0X800000)); // brick color
